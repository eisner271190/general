...
phases:
  pre_build:
    commands:
      ...
      # Keystore
      - 'aws secretsmanager get-secret-value --secret-id "/epc/com.quizsmart.app/android-signing/keystore" --query SecretBinary --output text | base64 --decode > "$CODEBUILD_SRC_DIR/frontend/quizsmart/android/upload-keystore.jks" && echo "Keystore OK: $(wc -c < "$CODEBUILD_SRC_DIR/frontend/quizsmart/android/upload-keystore.jks") bytes"'
      # key.properties
      - 'STORE_PW=$(aws secretsmanager get-secret-value --secret-id "/epc/com.quizsmart.app/android-signing/store-password" --query SecretString --output text) && KEY_PW=$(aws secretsmanager get-secret-value --secret-id "/epc/com.quizsmart.app/android-signing/key-password" --query SecretString --output text) && KEY_ALIAS=$(aws secretsmanager get-secret-value --secret-id "/epc/com.quizsmart.app/android-signing/key-alias" --query SecretString --output text) && printf "storePassword=%s\nkeyPassword=%s\nkeyAlias=%s\nstoreFile=upload-keystore.jks\n" "$STORE_PW" "$KEY_PW" "$KEY_ALIAS" | tee "$CODEBUILD_SRC_DIR/frontend/quizsmart/android/key.properties" && echo "key.properties OK"'
